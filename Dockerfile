...
RUN echo "====== INSTALL GCC-CROSS ======" \
 && dpkg --add-architecture arm64 && dpkg --add-architecture armhf && dpkg --add-architecture i386 && dpkg --add-architecture riscv64 \
 ...
 && apt-get -o Dpkg::Options::="--force-confnew" install -y --no-install-recommends \
  crossbuild-essential-arm64 crossbuild-essential-armhf crossbuild-essential-i386 crossbuild-essential-riscv64 \
  dpkg-dev dpkg-sig \
